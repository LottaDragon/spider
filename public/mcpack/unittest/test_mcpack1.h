 ...
 * $Id: test_mcpack1.h,v 1.1.2.2 2008/12/08 11:03:30 yufan Exp $ 
 * 
 **************************************************************************/
...
public:
	void test_array()
	{
		...
		mc_pack_t *p;
		{
			p = mc_pack_open_w(1, buf, sizeof(buf), tmpbuf, sizeof(tmpbuf));
	...
	void test_create()
	{
		...
		{
			mc_pack_t * pack = mc_pack_open_w(1, buf, sizeof(buf), tmp, sizeof(tmp));
	...
	void test_2text()
	{
		...
		TS_ASSERT_EQUALS(strlen(textbuf), 210);
		memset(buf, 0xff, sizeof(buf));
		{
			mc_pack_t * pack = mc_pack_open_w(1, buf, sizeof(buf), tmp, sizeof(tmp));
