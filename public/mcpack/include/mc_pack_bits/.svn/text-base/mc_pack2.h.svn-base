...
*   detail description
*  用于将Mcpack2类型转换为Mcpack1类型的Translate table
*/
...
public:
	...
	type_conversion()
	{
		...
		dict[MCPACKV2_DOUBLE] = MC_IT_DOUBLE;
...
* @brief short description
*   detail description
*  用于将Mcpack1类型转换为Mcpack2类型的Translate table
*/
...
public:
	...
	reverse_type_conversion()
	{
		...
		dict[MC_IT_DOUBLE] = MCPACKV2_DOUBLE;
...
class Protocol_v2 : public Mcpack
{
protected:
	typedef unsigned long long int hash_key_t;		   ///<字符串的hash值类型
	typedef inner_hash <hash_key_t, void *> index_t;		  ///<用于建立数组的索引  
	typedef inner_map <void *> map_t;		///<用于建立对象索引 
	void * _indexer;
	Protocol_v2 * _parent;
	unsigned char type;
	...
	inline int trans_number (const void * data, char type, double & value);
	...
	inline int Protocol_v2<alloc_t>::get_int_index(size_t in, mc_int64_t * value)
	{
		return get_number_index(in, value);
	}
...
public:
	...
	virtual int get_double_from_item(const mc_pack_item_t * item, double *res);
	...
	virtual int put_double(const char *name, double value);
	virtual int get_double(const char *name, double * value);
	virtual int get_double_index(size_t i, double * value);
	virtual int mod_double(const char *name, double value);
	virtual int mod_double_index(size_t i, double value);
...
template <typename alloc_t>
template <>
void * Protocol_v2<alloc_t>::make_space (u_int need_size);
*/
/**
 * @brief 
 * 检查指针是否超出有效范围
 * @param [in/out] ptr   : const void*
 * @return  template <typename alloc_t> int 
 * @retval   
 * @see 
 * @note 
 * @author yufan
 * @date 2008/12/04 16:25:46
**/
template <typename alloc_t>
 ...
 * @brief 
 * 修改所有父节点的大小
 * @param [in/out] itemsize   : u_int
 * @return  template <typename alloc_t> void 
 * @retval   
 * @see 
 * @note 
 * @author yufan
 * @date 2008/12/04 16:16:19
**/
template <typename alloc_t>
 ...
 * @brief 
 * 腾出空间，必要时可以移动内存
 * @param [in/out] need_size   : u_int
 * @return  template <typename alloc_t> template <typename T> T* 
 * @retval   
 * @see 
 * @note 
 * @author yufan
 * @date 2008/12/04 16:16:57
**/
template <typename alloc_t>
template <typename T>
...
{
	/*MCPACKV2_SHORT_ITEM * pack = (MCPACKV2_SHORT_ITEM *)_pack_ptr;
	(if(!(pack->item_count & 0x1))
	{
	return pack->content;
	}
	else
 ...
 * @brief 
 * 增加pack的item个数
 * @return  template <typename alloc_t> inline void 
 * @retval   
 * @see 
 * @note 
 * @author yufan
 * @date 2008/12/04 16:18:26
**/
template <typename alloc_t>
 ...
 * @brief 
 * 计算需要的索引大小
 * @return  template <typename alloc_t> inline int 
 * @retval   
 * @see 
 * @note 
 * @author yufan
 * @date 2008/12/04 16:19:24
**/
template <typename alloc_t>
 ...
 * @brief 
 * 将Mcpack中的数据标识为64位整数
 * @param [in/out] data   : const void*
 * @param [in/out] idtype   : char
...
inline int Protocol_v2<alloc_t>::trans_number (const void * data, char idtype, mc_int64_t & value)
{
	...
	case MCPACKV2_DOUBLE:
		if (value = (mc_int64_t)*(double *)data, value != *(double *)data)
		{
			return MC_PE_FOUND_BUT_TYPE;
		}
		else
...
template <typename alloc_t>
inline int Protocol_v2<alloc_t>::trans_number (const void * data, char idtype, double & value)
{
	switch(idtype & 0x7f)
	{
	case MCPACKV2_INT_8:
...
inline int Protocol_v2<alloc_t>::count () const
{
	...
	if(!(pack->item_count & 0x1))
	{
	return ((long_pack *)pack)->item_count >> 2;
	}
	else
	{*/
...
* @brief short description
*   detail description
*  各种整数类型的traits, 包括Mcpack类型id，是否是最大表示类型，范围
*  用于模板特化
*/
template<typename T>
...
{
	static const mc_pack_type_id TYPE_ID = MCPACKV2_DOUBLE;
};
...
{
	if(!(this->type == MCPACKV2_OBJECT && name != 0 || name == 0))
	{
		return MC_PE_BAD_TYPE;
	}
...
/**
 * @brief 
 * 获取整数值的实际方法，会被特化成有符号和无符号版本
 * @param [in/out] name   : char const*
 * @param [in/out] value   : T*
 * @return  template <typename alloc_t> template <typename T> int 
 * @retval   
 * @see 
 * @note 
 * @author yufan
 * @date 2008/12/04 16:27:15
**/
template <typename alloc_t>
template <typename T>
...
/**
 * @brief 
 * 按索引获取整数的实际方法，实力化为有符号和无符号版本
 * @param [in/out] in   : size_t
 * @param [in/out] value   : T*
 * @return  template <typename alloc_t> template <typename T> int 
 * @retval   
 * @see 
...
int Protocol_v2<alloc_t>::read(const char * __buf, u_int __size, alloc_t * alloc, Protocol_v2<alloc_t> ** __ppack)
{
	...
	control * con = alloc->template mallco<typename Protocol_v2<alloc_t>::control>();
	con->header = header;
	con->alloc = alloc;
	con->mode = READ_ONLY;
	Protocol_v2<alloc_t> * pack = alloc->template malloc<Protocol_v2<alloc_t> >();
 ...
 * @brief 
 * 计算hash值，算法可替换
 * @param [in/out] key   : char const*
 * @param [in/out] len   : int
 * @return  template <typename alloc_t> typename Protocol_v2<alloc_t>::hash_key_t 
 * @retval   
 * @see 
 * @note 
 * @author yufan
 * @date 2008/12/08 18:45:59
**/
template <typename alloc_t>
 ...
 * @brief 
 * 建立索引
 * @param [in/out] index   : index_t*
 * @return  template <typename alloc_t> void 
 * @retval   
 * @see 
 * @note 
 * @author yufan
 * @date 2008/12/08 18:46:24
**/
template <typename alloc_t>
 ...
 * @brief 
 * 为数组建立索引
 * @param [in/out] index   : map_t*
 * @return  template <typename alloc_t> void 
 * @retval   
 * @see 
 * @note 
 * @author yufan
 * @date 2008/12/08 18:46:40
**/
template <typename alloc_t>
 ...
 * @brief 
 * 按序号检索，如果索引可用使用索引
 * @param [in/out] i   : size_t
 * @return  template <typename alloc_t> void* 
 * @retval   
 * @see 
 * @note 
 * @author yufan
 * @date 2008/12/08 18:48:04
**/
template <typename alloc_t>
 ...
 * 修改简单字段的值
 * @param [in/out] name   : char const*
 * @param [in/out] value   : void const*
 * @param [in/out] len   : size_t
 * @param [in/out] idtype   : mc_pack_type_id
 * @return  template <typename alloc_t> int 
 * @retval   
 * @see 
 * @note 
 * @author yufan
 * @date 2008/12/08 18:49:14
**/
template <typename alloc_t>
 ...
 * @param [in/out] len   : int
 * @param [in/out] idtype   : mc_pack_type_id
 * @param [in/out] titem   : long_vitem**
 * @param [in/out] content   : void**
 * @return  template <typename alloc_t> int 
 * @retval   
 * @see 
 * @note 
 * @author yufan
 * @date 2008/12/08 18:50:10
**/
template <typename alloc_t>
 ...
 * @param [in/out] name   : char const*
 * @param [in/out] idtype   : mc_pack_type_id
 * @param [in/out] out_pack   : Mcpack**
 * @return  template <typename alloc_t> int 
 * @retval   
 * @see 
 * @note 
 * @author yufan
 * @date 2008/12/08 18:50:31
**/
template <typename alloc_t>
 ...
 * @brief  
 * 创建Mcpack_item
 * @param [in/out] ptr   : const char*
 * @param [in/out] __item   : mc_pack_item_t*
 * @return  template <typename alloc_t> int 
 * @retval   
 * @see 
 * @note 
 * @author yufan
 * @date 2008/12/04 16:29:14
**/
template <typename alloc_t>
...
int Protocol_v2<alloc_t>::next_item(const mc_pack_item_t * __itemold, mc_pack_item_t * __item)
{
	...
	char *ptr;
	char oldtype = greverse_type_dict.dict[__itemold->type];
	if(oldtype == MCPACKV2_OBJECT || oldtype == MCPACKV2_ARRAY)
	{
		int sl = strlen(__itemold->key);
		if(sl > 0 )
			sl++;
		long_vitem * titem = (long_vitem *)((char *)__itemold->value - sl - sizeof(long_vitem));
		ptr = (char *)__itemold->value + titem->content_len;
	}
	else
	{
		ptr = (char *)__itemold->value + __itemold->value_size;
	}
	while(true)
	{
		if(check_ptr(ptr + sizeof(short_item)) < 0)
...
template <typename alloc_t>
int Protocol_v2<alloc_t>::get_double_from_item(const mc_pack_item_t * mcitem, double *value)
{
	if(0 == mcitem || 0 == value)
	{
		return MC_PE_BAD_PARAM;
...
int Protocol_v2<alloc_t>::get_pack_from_item(const mc_pack_item_t * item, Mcpack **out_pack)
{
	...
	if(pack == NULL)
		return MC_PE_NO_TEMP_SPACE;
	int sl = strlen(item->key);
	if(sl > 0 )
		sl++;
	long_vitem * titem = (long_vitem *)((char *)item->value - sl - sizeof(long_vitem));
	if(titem != 0)
	{
		if(titem->type == MCPACKV2_NULL)
		{
...
int Protocol_v2<alloc_t>::put_item(const mc_pack_item_t * item)
{
	...
	case MC_IT_DOUBLE:
		return put_double(name, *(double *)item->value);
	case MC_IT_BOOL:
...
int Protocol_v2<alloc_t>::foreach(mc_pack_foreach_callback cb, void *arg)
{
	...
	while(MC_PE_SUCCESS == result)
	{
		...
		{
			Mcpack * p;
			if(result = get_pack_from_item(&item, &p) < 0)
			{
				return result;
			}
			else
		...
		{
			Mcpack * p;
			if(result = get_array_from_item(&item, &p) < 0)
			{
				return result;
			}
			else
...
int Protocol_v2<alloc_t>::copy_item(const Mcpack *pack, const char *name)
{
	...
	if(((Protocol_v2<alloc_t> *)pack)->type != MCPACKV2_OBJECT)
		return MC_PE_WRONG_PACK_TYPE;
...
int Protocol_v2<alloc_t>::copy_item_index(const Mcpack *pack, size_t index)
{
	if(pack->version() != 2)
		return MC_PE_PROTOCOL_NOT_SUPPOT_OPERATION;
	if(((Protocol_v2<alloc_t> *)pack)->type != MCPACKV2_ARRAY)
		return MC_PE_WRONG_PACK_TYPE;
...
int Protocol_v2<alloc_t>::copy_all_item(const Mcpack *pack)
{
	if(pack->version() != 2)
		return MC_PE_PROTOCOL_NOT_SUPPOT_OPERATION;
	if(((Protocol_v2<alloc_t> *)pack)->type != this->type)
		return MC_PE_WRONG_PACK_TYPE;
...
template <typename alloc_t>
int Protocol_v2<alloc_t>::put_double (char const * name, double value)
{
	return put_simple_item(name, &value);
}

template <typename alloc_t>
int Protocol_v2<alloc_t>::get_double (char const * name, double *value)
{
	return get_number(name,value);
}

template <typename alloc_t>
int Protocol_v2<alloc_t>::get_double_index (size_t in, double * value)
{
	return get_number_index(in, value);
}

template <typename alloc_t>
int Protocol_v2<alloc_t>::mod_double_index (size_t in, double value)
{
	return mod_number_index(in, value);
}

template <typename alloc_t>
int Protocol_v2<alloc_t>::mod_double (char const * name, double value)
{
	return mod_number(name, value);
}
