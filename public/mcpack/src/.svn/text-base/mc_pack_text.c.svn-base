 ...
 * $Id: mc_pack_text.c,v 1.1.2.15 2009/03/09 08:02:52 yufan Exp $ 
 * 
 **************************************************************************/
	...
	switch (pitem->type) {
		...
		case MC_IT_FLOAT:
			...
			if (json) {
				...
				BUF_PRINT(buf, size, used, "(float)%f", *(float*)pitem->value);
			}
			break;
		case MC_IT_DOUBLE:
			if (! is_arr) {
			...
			if (json) {
				...
				BUF_PRINT(buf, size, used, "(double)%lf", *(double*)pitem->value);
			}
			break;
		default:
			return 0;
