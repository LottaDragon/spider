 ...
 * $Id: mc_pack_ex.c,v 1.1.2.11 2009/03/13 09:48:22 yufan Exp $ 
 * 
 **************************************************************************/
...
static const char *ERR_STR[] = {
	...
	"MCP: Run out of temporary buffer",
	"MCP: Operation not permitted in this read/write mode",
	"MCP: Operation not permitted in this object/array pack",
	"MCP: Operation not supported by underling protocol",
	"MCP: Unknown"
};
...
{
	if (err <= 0 && err > -(int)(sizeof(ERR_STR)/sizeof(ERR_STR[0]))) {
		return ERR_STR[-err];
	}
...
mc_pack_creatf(int version, char *buf, u_int buf_size, char * tmp, u_int tmp_size, const char *fmt, ...)
{
	...
	pack = mc_pack_open_w(version, buf, buf_size, tmp, tmp_size);
...
static int output_pack(const mc_pack_t *ppack, char *str, u_int size)
{
	...
	while (MC_PE_SUCCESS == ret) {
		...
		{
			const mc_pack_t *p;
			int result = mc_pack_get_pack_from_item(&itm, &p);
			if(result < 0)
				return INT32_MAX;
			ret = output_pack(p, str + used, (u_int)size_left);
		}
