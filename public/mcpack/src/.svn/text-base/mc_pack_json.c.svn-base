 ...
 * $Id: mc_pack_json.c,v 1.1.2.13 2009/03/24 06:05:20 yufan Exp $ 
 * 
 **************************************************************************/
...
int
mc_pack_json2pack(const char *txt, mc_pack_t *pack)
{
    ...
    token_q = malloc(tok_cnt * sizeof(token_t));
    if(token_q == 0)
    {
        return MC_PE_BAD_PARAM;
    }
...
static int
print_item(const mc_pack_item_t *pitem, char *buf, int size, int is_arr)
{
	int used = 0;
	switch (pitem->type) {
		case MC_IT_TXT:
...
static int
output_pack(const mc_pack_t *pack, char *str, int size, int is_arr)
{
	mc_pack_item_t itm;
	int ret = 0;
	int is_first = 1;
	...
	while (MC_PE_SUCCESS == ret) {
		...
		if (MC_PT_OBJ == itm.type) {
			...
			if(result < 0)
				return size;
			used += output_pack(p, str + used, size - used, 0);
			BUF_PRINT(str, size, used, "}");
			...
			if(result < 0)
				return size;
			used += output_pack(p, str + used, size - used, 1);
			BUF_PRINT(str, size, used, "]");
		} else {
			used += print_item(&itm, str + used, size - used, is_arr);
		}
...
int
mc_pack_pack2json(const mc_pack_t *pack, char *txt_buf, u_int buf_size)
{
	u_int used = 0;
	...
	if (used < buf_size) {
		used += output_pack(pack, txt_buf + used, buf_size - used, 0);
		if (used < buf_size) {
			used += snprintf(txt_buf + used, buf_size - used, "}");
		}
	}
