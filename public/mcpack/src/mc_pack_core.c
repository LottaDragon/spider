 ...
 * $Id: mc_pack_core.c,v 1.1.2.14 2009/03/11 11:03:13 yufan Exp $ 
 * 
 **************************************************************************/
...
**/
static inline int
get_type(const mc_v1pack_t * ppack)
{
	if (NULL == ppack) {
		return MC_PT_BAD;
	}
...
**/
static inline int
get_full_name(const mc_v1pack_t *ppack, const char *name, char *full_name, u_int size)
{
	u_int used = 0;
...
get_item(const mc_v1pack_t * ppack, const char *name)
{
...
**/
static mc_v1pack_t *
put_item(mc_v1pack_t * ppack, const char *name, int type, const char *data, u_int size)
{
	char full_name[MC_PACK_FIELD_NAME_LEN_MAX];		  /* 考虑效率，没有初始化       */
	int pack_type = 0;
	internal_item_t *pitem = NULL;
...
mc_v1pack_t *
mc_v1pack_create(char *buf, u_int buf_size)
{
...
int
mc_v1pack_get_size(const mc_v1pack_t * ppack)
{
	if (NULL == ppack) {
...
int
mc_v1pack_get_item_count(const mc_v1pack_t * ppack)
{
	int pack_type = 0;
...
mc_v1pack_t *
mc_v1pack_put_array(mc_v1pack_t * ppack, const char *name)
{
	char idx_name[16];
	u_int idx;
...
mc_v1pack_t *
mc_v1pack_put_object(mc_v1pack_t * ppack, const char *name)
{
	char idx_name[16];
	u_int idx;
...
mc_v1pack_t *
mc_v1pack_get_array(const mc_v1pack_t * ppack, const char *name)
{
	long err = 0;
	mc_v1pack_t *ap = NULL;
...
mc_v1pack_t *
mc_v1pack_get_array_arr(const mc_v1pack_t * ppack, u_int arr_index)
{
	char name[16];
	long err = 0;
	mc_v1pack_t *ap = NULL;
...
mc_v1pack_t *
mc_v1pack_get_object(const mc_v1pack_t * ppack, const char *name)
{
	long err = 0;
	mc_v1pack_t *op = NULL;
	const mc_v1pack_t *ppack_root = ppack;
...
mc_v1pack_t *
mc_v1pack_get_object_arr(const mc_v1pack_t * ppack, u_int arr_index)
{
	mc_v1pack_t *op = NULL;
	const mc_v1pack_t *ppack_root = NULL;
	long err = 0;
...
int
mc_v1pack_valid(const mc_v1pack_t * ppack, u_int buf_size)
{
	int ret = 0;
...
int
mc_v1pack_put_null(mc_v1pack_t * ppack, const char *name)
{
	...
	if (IS_ARRAY(ppack)) {
		...
		idx = AO_ITEM_COUNT(ppack);
		snprintf(idx_name, sizeof(idx_name), "%u", idx);
		ret = put_item(ppack, idx_name, MC_IT_NULL, 0, 0);
...
int
mc_v1pack_put_int32(mc_v1pack_t * ppack, const char *name, mc_int32_t value)
{
	mc_v1pack_t *ret = NULL;
	char idx_name[16];
	u_int idx;
...
int mc_v1pack_put_float(mc_v1pack_t * ppack, const char *name, float value)
{
	...
	if (IS_ARRAY(ppack)) {
		...
		idx = AO_ITEM_COUNT(ppack);
		snprintf(idx_name, sizeof(idx_name), "%u", idx);
		ret = put_item(ppack, idx_name, MC_IT_FLOAT, (const char *) &value, sizeof(float));
		return MC_PACK_PTR_ERR(ret);
	}
...
int mc_v1pack_put_double(mc_v1pack_t * ppack, const char *name, double value)
{
	...
	if (IS_ARRAY(ppack)) {
		...
		idx = AO_ITEM_COUNT(ppack);
		snprintf(idx_name, sizeof(idx_name), "%u", idx);
		ret = put_item(ppack, idx_name, MC_IT_DOUBLE, (const char *) &value, sizeof(double));
		return MC_PACK_PTR_ERR(ret);
	}
...
int mc_v1pack_put_bool(mc_v1pack_t * ppack, const char *name, mc_bool_t value)
{
	...
	if (IS_ARRAY(ppack)) {
		...
		idx = AO_ITEM_COUNT(ppack);
		snprintf(idx_name, sizeof(idx_name), "%u", idx);
		ret = put_item(ppack, idx_name, MC_IT_BOOL, (const char *) &value, sizeof(mc_bool_t));
		return MC_PACK_PTR_ERR(ret);
	}
	p = (value != 0); 
...
int
mc_v1pack_put_uint32(mc_v1pack_t * ppack, const char *name, mc_uint32_t value)
{
	mc_v1pack_t *ret = NULL;
	char idx_name[16];
	u_int idx;
...
int
mc_v1pack_put_int64(mc_v1pack_t * ppack, const char *name, mc_int64_t value)
{
	mc_v1pack_t *ret = NULL;
	char idx_name[16];
	u_int idx;
...
int
mc_v1pack_put_uint64(mc_v1pack_t * ppack, const char *name, mc_uint64_t value)
{
	mc_v1pack_t *ret = NULL;
	char idx_name[16];
	u_int idx;
...
static int
put_str(mc_v1pack_t *ppack, const char *name, const char *value, u_int size)
{
	mc_v1pack_t *ret = NULL;
	char idx_name[16];
	u_int idx;
...
int
mc_v1pack_put_str(mc_v1pack_t *ppack, const char *name, const char *value)
{
	if (NULL == value) {
		return MC_PE_BAD_PARAM;
	}
...
int
mc_v1pack_put_nstr(mc_v1pack_t *ppack, const char *name, const char *value, u_int len)
{
	u_int slen = 0;
...
int
mc_v1pack_put_raw(mc_v1pack_t * ppack, const char *name, const void *rawdata, u_int size)
{
	mc_v1pack_t *ret = NULL;
	char idx_name[16];
	u_int idx;
...
int
mc_v1pack_get_float(const mc_v1pack_t * ppack, const char *name, float * result)
{
...
int
mc_v1pack_get_double(const mc_v1pack_t * ppack, const char *name, double * result)
{
	internal_item_t *ip = NULL;
...
int
mc_v1pack_get_bool(const mc_v1pack_t * ppack, const char *name, mc_bool_t * result)
{
	internal_item_t *ip = NULL;
...
int
mc_v1pack_get_int32(const mc_v1pack_t * ppack, const char *name, mc_int32_t * result)
{
	internal_item_t *ip = NULL;
...
int
mc_v1pack_get_uint32(const mc_v1pack_t * ppack, const char *name, mc_uint32_t * result)
{
	internal_item_t *ip = NULL;
...
int
mc_v1pack_get_int64(const mc_v1pack_t * ppack, const char *name, mc_int64_t * result)
{
	internal_item_t *ip = NULL;
...
int
mc_v1pack_get_uint64(const mc_v1pack_t * ppack, const char *name, mc_uint64_t * result)
{
	internal_item_t *ip = NULL;
...
const char *
mc_v1pack_get_str(const mc_v1pack_t * ppack, const char *name)
{
	internal_item_t *ip = NULL;
...
const char *
mc_v1pack_get_str_def(const mc_v1pack_t * ppack, const char *name, const char *def_value)
{
	internal_item_t *ip = NULL;
...
const void *
mc_v1pack_get_raw(const mc_v1pack_t * ppack, const char *name, u_int *size)
{
	internal_item_t *ip = NULL;
...
int
mc_v1pack_get_int32_arr(const mc_v1pack_t * ppack, u_int arr_index, mc_int32_t * result)
{
	char name[16];
	internal_item_t *ip = NULL;
...
int
mc_v1pack_get_uint32_arr(const mc_v1pack_t * ppack, u_int arr_index, mc_uint32_t * result)
{
	char name[16];
	internal_item_t *ip = NULL;
...
int
mc_v1pack_get_int64_arr(const mc_v1pack_t * ppack, u_int arr_index, mc_int64_t * result)
{
	char name[16];
	internal_item_t *ip = NULL;
...
int
mc_v1pack_get_uint64_arr(const mc_v1pack_t * ppack, u_int arr_index, mc_uint64_t * result)
{
	char name[16];
	internal_item_t *ip = NULL;
...
int
mc_v1pack_get_float_arr(const mc_v1pack_t * ppack, u_int arr_index, float * result)
{
	char name[16];
	internal_item_t *ip = NULL;
...
int
mc_v1pack_get_double_arr(const mc_v1pack_t * ppack, u_int arr_index, double * result)
{
	char name[16];
	internal_item_t *ip = NULL;
...
int
mc_v1pack_get_bool_arr(const mc_v1pack_t * ppack, u_int arr_index, mc_bool_t * result)
{
	char name[16];
	internal_item_t *ip = NULL;
...
const char *
mc_v1pack_get_str_arr(const mc_v1pack_t * ppack, u_int arr_index)
{
	char name[16];
	internal_item_t *ip = NULL;
...
const char *
mc_v1pack_get_str_arr_def(const mc_v1pack_t * ppack, u_int arr_index, const char *def_value)
{
	char name[16];
	internal_item_t *ip = NULL;
...
const void *
mc_v1pack_get_raw_arr(const mc_v1pack_t * ppack, u_int arr_index, u_int *size)
{
	char name[16];
	internal_item_t *ip = NULL;
...
{
	while (get_type((mc_v1pack_t *)pitem) != MC_PT_BAD) {
		pitem->offset -= off;
...
**/
static int
item_del(mc_v1pack_t * ppack, const char *name)
{
	mc_v1pack_t *ppack_root = ppack;
	char full_name[MC_PACK_FIELD_NAME_LEN_MAX];		  /* 考虑效率，没有初始化       */
	int pack_type = 0;
	int ret = 0;
	char *p = NULL;
...
int
mc_v1pack_rm(mc_v1pack_t * ppack, const char *name)
{
...
int
mc_v1pack_first_item(const mc_v1pack_t * ppack, mc_pack_item_t * pitem)
{
	int pack_type = 0;
	int prefix_len = 0;
...
int
mc_v1pack_next_item(const mc_pack_item_t * pitem_now, mc_pack_item_t * pitem_out)
{
	int prefix_len = 0;
	char prefix[MC_PACK_FIELD_NAME_LEN_MAX];
	const internal_item_t *pitem_edit = NULL;
...
int
mc_v1pack_get_by_idx(const mc_v1pack_t * ppack, u_int idx, mc_pack_item_t * pitem)
{
	int ret = 0;
	u_int i = 0;
...
int
mc_v1pack_close(mc_v1pack_t * ppack)
{
	if (NULL == ppack) {
		return MC_PE_BAD_PARAM;
	}
...
int
mc_v1pack_reopen(mc_v1pack_t * ppack, u_int buf_size)
{
	if (NULL == ppack) {
		return MC_PE_BAD_PARAM;
	}
...
**/
static int
pack_copy(const mc_v1pack_t * src, mc_v1pack_t * dst, const char *name)
{
	int ret = 0;
	int ret1 = 0;
	mc_pack_item_t itm;
...
int
mc_v1pack_copy_item(const mc_v1pack_t *src_pack, mc_v1pack_t *dst_pack, const char *name)
{
	return pack_copy(src_pack, dst_pack, name);
}

int
mc_v1pack_copy_item_arr(const mc_v1pack_t *src_arr, mc_v1pack_t *dst_arr, u_int idx)
{
	char name[16];
...
int
mc_v1pack_get_item(const mc_v1pack_t * ppack, const char *name, mc_pack_item_t *pitem)
{
	internal_item_t *ip = NULL;
...
int
mc_v1pack_get_item_arr(const mc_v1pack_t * ppack, u_int idx, mc_pack_item_t *pitem)
{
	char name[16];
	internal_item_t *ip = NULL;
...
int
mc_v1pack_reset(mc_v1pack_t *ppack)
{
	if (get_type(ppack) != MC_PT_PCK) {
		return MC_PE_BAD_PARAM;
	}
...
int
mc_v1pack_foreach(const mc_v1pack_t *ppack, mc_v1pack_foreach_callback cb, void *arg)
{
	int ret = 0;
	int retcnt = 0;
	int pack_type = 0;
...
int
mc_v1pack_get_int32_from_item(const mc_pack_item_t *pitem, mc_int32_t *res)
{
	internal_item_t *ip = NULL;
...
int
mc_v1pack_get_uint32_from_item(const mc_pack_item_t *pitem, mc_uint32_t *res)
{
	internal_item_t *ip = NULL;
	if (NULL == pitem || NULL == res) {
		return MC_PE_BAD_PARAM;
...
int
mc_v1pack_get_int64_from_item(const mc_pack_item_t *pitem, mc_int64_t *res)
{
	internal_item_t *ip = NULL;
	if (NULL == pitem || NULL == res) {
		return MC_PE_BAD_PARAM;
...
int
mc_v1pack_get_uint64_from_item(const mc_pack_item_t *pitem, mc_uint64_t *res)
{
	internal_item_t *ip = NULL;
	if (NULL == pitem || NULL == res) {
		return MC_PE_BAD_PARAM;
...
int
mc_v1pack_get_bool_from_item(const mc_pack_item_t *pitem, mc_bool_t * res)
{
	if (NULL == pitem || NULL == res) 
	{
		return MC_PE_BAD_PARAM;
...
int
mc_v1pack_get_str_from_item(const mc_pack_item_t *pitem, char ** res)
{
	if (NULL == pitem || NULL == res) {
		return MC_PE_BAD_PARAM;
	}
...
int
mc_v1pack_get_float_from_item(const mc_pack_item_t *pitem, float *res)
{
	internal_item_t *ip = NULL;
	if (NULL == pitem || NULL == res) {
		return MC_PE_BAD_PARAM;
...
int
mc_v1pack_get_double_from_item(const mc_pack_item_t *pitem, double *res)
{
	internal_item_t *ip = NULL;
	if (NULL == pitem || NULL == res) {
		return MC_PE_BAD_PARAM;
...
int
mc_v1pack_put_item(mc_v1pack_t *pack, const mc_pack_item_t *pitem)
{
	mc_v1pack_t *ret = NULL;
