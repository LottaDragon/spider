 ...
 * $Id: ResourcePool.h,v 1.4 2009/06/26 07:24:13 zhangfang Exp $ 
 * 
 **************************************************************************/
...
namespace bsl{
    ...
    */
    ...
    public:
        ...
        **/
        ResourcePool()
            :_mempool(_s_syspool), _p_attach_object_list(NULL), _p_attach_array_list(NULL), _p_alloc_object_list(NULL), _p_alloc_array_list(NULL){}
        ...
        **/
        void reset(){
            for (block_list_node_t<attach_object_info_t> *p_node = _p_attach_object_list; NULL != p_node; p_node = p_node->p_next ){
                for( attach_object_info_t* p_item = p_node->data + p_node->current; p_item >= p_node->data; --p_item ){
                    p_item->destructor(p_item->p_object);
                }
            }
            _clear_info_list(_p_attach_object_list);
            for (block_list_node_t<attach_array_info_t> *p_node = _p_attach_array_list; NULL != p_node; p_node = p_node->p_next ){
                for( attach_array_info_t* p_item = p_node->data + p_node->current; p_item >= p_node->data; --p_item ){
                    p_item->destructor(p_item->begin, p_item->end);
                }
            }
            _clear_info_list(_p_attach_array_list);
            for (block_list_node_t<alloc_object_info_t> *p_node = _p_alloc_object_list; NULL != p_node; p_node = p_node->p_next ){
                for( alloc_object_info_t* p_item = p_node->data + p_node->current; p_item >= p_node->data; --p_item ){
                    p_item->destructor(p_item->p_object, _mempool);
                }
            }
            _clear_info_list(_p_alloc_object_list);
            for (block_list_node_t<alloc_array_info_t> *p_node = _p_alloc_array_list; NULL != p_node; p_node = p_node->p_next ){
                for( alloc_array_info_t* p_item = p_node->data + p_node->current; p_item >= p_node->data; --p_item ){
    ...
    private:
        ...
        * @brief 空间适配器
        *  
        *  TODO:将来可以使用EBO技术进行优化。，但考虑到ResourcePool不会有很多，优化的意义不大。
        */
        mempool&    _mempool;
        /**
